...
result if you knew the right one before you ran it. Follow `/viby-toolkit:principles`.
...
- **Throwaway worktree** (`/viby-toolkit:worktrees`) or a scratch dir, an isolated env.
...
- **Where does the data go?** A candidate that ships your source or your users' data to a third party
  fails on that alone, whatever it scores. Name every egress and how to turn it off.
- **Who maintains it?** Dispatch a `researcher` rather than reading repos inline. One-maintainer
  projects, no commits in months, and a star count outrunning the commit history are risk signals —
  record them as accepted cost, never as adoption evidence.
- **What is the blast radius if it is wrong?** A planning aid that misleads costs an hour; something in
  the path of auth, payments or migrations costs more, so the correctness bar rises with it.

## Output — the decision record

Save it to `docs/decisions/<date>-<topic>.md` (or the project's convention), opening with a **status
line** stating what was actually done — a document that cannot fill that line honestly is a survey,
not an evaluation, and should be labelled as one. The full section-by-section template, the worked
status-line example, and the expanded gate checklist are in
`references/gates-and-decision-record.md`.

Then record the durable part with `/viby-toolkit:learn` — a rejected-option-and-why is exactly the
lesson a future session would otherwise pay to re-derive.
