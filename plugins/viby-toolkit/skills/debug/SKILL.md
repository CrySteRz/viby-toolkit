...
**If it is broken in production right now, start with `/viby-toolkit:incident` instead.** That
...
`/viby-toolkit:principles`. Tempted to skip straight to a fix under time pressure? Don't —
...
typo or setup error), and keep it. `/viby-toolkit:test` covers how to make it a *good* test —
...
`/viby-toolkit:verify`, which also screens for the silent-pass modes that make a fix look
