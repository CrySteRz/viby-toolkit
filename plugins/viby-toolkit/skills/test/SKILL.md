...
- **A flaky test gets root-caused (`/viby-toolkit:debug`) or quarantined with an owner — never
...
- **Check the suite actually runs.** Zero collected, all-skipped, and excluded directories
  all report green (see `/viby-toolkit:verify` §4).
...
`/viby-toolkit:learn` so the next session doesn't rediscover it.
