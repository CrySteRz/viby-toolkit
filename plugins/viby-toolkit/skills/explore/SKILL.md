...
**Stamp it — a map is a derived artifact** (`/viby-toolkit:principles` §9): put the commit
SHA and date it was built from at the top, and the fact that it is regenerated by re-running
this skill, not hand-patched. An unstamped map has no way to be found stale, so it gets
believed long after the code moved; a stamped one gets checked against `git log` in seconds.
Include:
