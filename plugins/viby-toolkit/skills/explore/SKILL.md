...
`/viby-toolkit:principles` — this is pure read work, so **fan out**.
...
- If the task is now clear, hand to `/viby-toolkit:plan` or `/viby-toolkit:orchestrate` — the map
  is your research phase, so do not redo it.
- Record anything durable and non-obvious with `/viby-toolkit:learn` (a build quirk, a
