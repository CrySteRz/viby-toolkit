...
Make the smallest change you want; when it breaks something, **revert it** and note the prerequisite
that has to land first. Work the prerequisites depth-first until one applies cleanly, then unwind. The
suite stays green after every step, so you are never holding a half-migrated codebase you cannot ship
or abandon. Full procedure, including how to keep the graph when context is compacted away:
`references/mikado-steps.md`.
