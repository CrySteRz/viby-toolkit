---
description: Run the full forge pipeline autonomously on a task — orchestrate, build, verify, self-review with the false-positive filter — and don't stop until it's verified done.
---

Take the following task and drive it all the way to a verified, self-reviewed result using
the forge workflow. Be autonomous: make reasonable decisions and proceed rather than
...
1. Invoke the `/forge:orchestrate` skill and follow its full pipeline (scope → explore →
...
3. In the self-review phase, run `/forge:review-cluster` on your own diff and fix every
   **confirmed** finding (re-verifying each fix). Surface any unconfirmed findings for the
   user to judge.
4. Follow `/forge:forge-principles` throughout: fan out cheap scout agents for breadth,
