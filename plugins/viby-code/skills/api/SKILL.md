...
Follow `/viby-code:principles`. This is a doctrine skill: it encodes established interface
...
- **Errors are part of the contract.** Define the shape (code, message, and a stable
  ...
  and a security finding (`/viby-code:secure`).
...
- **Expand then contract**, exactly as in `/viby-code:schema`: add the new shape, support both,
...
- Consult `/viby-code:release` when the change lands: the version number is the promise you
