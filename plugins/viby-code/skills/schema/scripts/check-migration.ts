/**
 * viby-code migration safety linter — the executable half of /viby-code:schema.
 *
  ...
  if (!values.quiet) {
    ...
    console.log(
      "Engine and version change the details — verify each against YOUR database before acting,\n" +
        "and never run an unreviewed migration against production data. See /viby-code:schema.",
    );
  }
  return findings.length > 0 ? 1 : 0;
}
