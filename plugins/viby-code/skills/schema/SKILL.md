...
description: >
  ...
  /viby-code:migrate, which sweeps code, not data.
---
...
Follow `/viby-code:principles`. This skill has its own Iron Law because schema work breaks
...
```bash
CHECK=$(ls "$HOME"/.claude/plugins/cache/*/viby-code/*/skills/schema/scripts/check-migration.ts 2>/dev/null | tail -1)
...
- After running: verify the schema is what you intended, row counts are what you expected, the
  application works against it (`/viby-code:verify`), and — for a backfill — that no rows were
