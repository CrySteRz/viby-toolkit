...
description: >
  ...
  "prove it", "are the tests passing", or when you're about to write "should work" /
  "that should fix it". Also the Verify phase of /viby-code:orchestrate and the closing
  step of /viby-code:debug and /viby-code:migrate.
---
...
has. This skill replaces that signal with evidence. Follow `/viby-code:principles` §5.
...
- If a project memory or CLAUDE.md records the command (see `/viby-code:learn`), use it —
...
- Two failed correction attempts on the same issue → stop patching. Reassess (and consider
  `/viby-code:debug` for a real root-cause pass) rather than iterating blind.
