...
- **You have a clear, well-specified ticket or spec** — unambiguous scope, known
  acceptance criteria, no real fork in the road. → Go straight to `/viby-code:plan` (design the
  HOW) or `/viby-code:orchestrate` (which plans then builds). The person who wrote the ticket
...
it's the right thing. Follow `/viby-code:principles`. The mistakes this prevents —
