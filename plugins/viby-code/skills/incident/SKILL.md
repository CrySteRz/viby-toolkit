...
description: >
  ...
  rate spiked", "it's timing out in prod". Distinct from /viby-code:debug, which assumes you
  have time to reproduce; here the order of operations is different.
---
...
Follow `/viby-code:principles`, with one deliberate inversion: `/viby-code:debug` forbids a fix
...
Now the pressure is off, so switch to `/viby-code:debug` and its discipline: reproduce it (the
...
- **Fix the root cause** with normal rigour: a regression test that reproduces it
  (`/viby-code:test`), the fix, verification (`/viby-code:verify`).
- **Fix the detection.** If users told you before monitoring did, that is its own defect.
- **Record it** with `/viby-code:learn` — the symptom, the cause, the mitigation that worked.
...
- Ask what made this *possible*, not just what made it happen: no rollback path, no flag, an
  ...
  `/viby-code:schema` and `/viby-code:release` exist largely to prevent this class.
