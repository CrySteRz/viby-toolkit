...
description: >
  ...
  naming", "make this testable", "untangle this". Distinct from /viby-code:migrate (a
...
Follow `/viby-code:principles`. Reading the code is fan-out work; the restructuring itself is
a write, so it stays single-threaded.

## Where this sits

- `refactor` — same behaviour, better structure. **This skill.**
- `/viby-code:migrate` — one mechanical transformation applied across many files.
- `/viby-code:perf` — behaviour preserved, but the *point* is a measured speed change.
- `/simplify` — small local cleanups of code you just wrote.
- A **rewrite** changes behaviour. That is a feature, and belongs in
  `/viby-code:orchestrate` with a plan and tests, not here.
...
- Check whether they actually cover the behaviour you are about to move, not just that they
  exist. `/viby-code:test` covers judging that; the scanner will tell you if those tests
  assert nothing.
