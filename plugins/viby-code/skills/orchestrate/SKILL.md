...
If the **what** itself is unsettled (a fuzzy idea, not a spec), run `/viby-code:brainstorm`
...
- Honor the **escalation ladder** (`/viby-code:principles`): if a scout returns
...
change is risky. Use `/viby-code:plan` if the planning itself is hard enough to warrant a
...
Run `/viby-code:review-cluster` on your own diff — a fresh-context reviewer is unbiased toward
...
repo insists on), invoke `/viby-code:learn` to record it to project memory so the next session
gets it for free.
