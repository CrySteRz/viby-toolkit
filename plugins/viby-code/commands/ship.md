...
the viby-code workflow. Be autonomous: make reasonable decisions and proceed rather than
...
3. In the self-review phase, run `/viby-code:review-cluster` on your own diff and fix every
   **confirmed** finding (re-verifying each fix). Surface any unconfirmed findings for the
   user to judge.
4. Follow `/viby-code:principles` throughout: fan out cheap scout agents for breadth,
